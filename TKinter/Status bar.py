...
status = Label(root, text='Image 1 of ' + str(len(image_list)), bd=1, relief=SUNKEN, anchor=E)#bd creates a border of the widget #releif=SUNKEN - makes the widget looks sunk in the program #anchor - you can anchor a widget in a direction
...
def forward(image_number):
    ...
    status = Label(root, text='Image ' + str(image_number) + ' of ' + str(len(image_list)), bd=1, relief=SUNKEN, anchor=E) #Adding this to the function so that it updates the number with it as the number updates in the funciton
    status.grid(row=2, column=0, columnspan=3, sticky=W+E)#Adding this to the function so that it updates the number with it
...
status.grid(row=2, column=0, columnspan=3, sticky=W+E)#The grid system has a navigational system based on North South East and West, we can tell this to stretch in any direction. The sticky option specifies which edge of the cell the widget should stick to.
