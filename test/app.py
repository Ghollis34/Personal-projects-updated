...
async def home(request: Request):
    ...
    meals = conn.execute(
        'SELECT meal, fats, carbs, protein, calories FROM meals_and_ingredients ORDER BY meal').fetchall()
    conn.close()
...
async def add_meal(request: Request):
    ...
    conn = get_db_connection()
    conn.execute('INSERT INTO meals_and_ingredients (meal, ingredients, Fats, Carbs, Protein, Calories) VALUES (?, ?, ?, ?, ?, ?)',
                 (meal, ingredients, fats, carbs, protein, calories))
    conn.commit()
    conn.close()
...
async def edit_meal(request: Request):
    conn = get_db_connection()
    meals = conn.execute(
        'SELECT meal, ingredients FROM meals_and_ingredients ORDER BY meal').fetchall()
...
async def edit_meal(request: Request):
    ...
    if 'delete_meal' in form_data:
        ...
        conn = get_db_connection()
        conn.execute(
            'DELETE FROM meals_and_ingredients WHERE meal=?', (meal_to_delete,))
        conn.commit()
        conn.close()
        ...
        new_fats = form_data['new_fats']
        new_carbs = form_data['new_carbs']
        new_protein = form_data['new_protein']
        new_calories = form_data['new_calories']
        conn = get_db_connection()
        conn.execute('UPDATE meals_and_ingredients SET meal=?, ingredients=?, Fats=?, Carbs=?, Protein=?, Calories=? WHERE meal=?',
                     (new_meal_name, new_ingredients, new_fats, new_carbs, new_protein, new_calories, meal_to_edit))
        conn.commit()
        conn.close()
...
async def generate_ingredients(request: Request):
    ...
    for meal in meals:
        if meal:
            conn = get_db_connection()
            result = conn.execute(
                'SELECT ingredients FROM meals_and_ingredients WHERE meal=?', (meal,))
            if result:
                ingredients = result.fetchone()
                if ingredients:
                    ingredients_list.extend([ingredient.strip()
                                            for ingredient in ingredients['ingredients'].split(",")])
            conn.close()
